...
<head>
    ...
    <title>N.I.U.S. | Intelligence Source</title>
    ...
    <meta property="og:description" content="Automated intelligence reports for the modern IT professional.">
    ...
    <style>
        * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
        ...
        /* TERMINAL MODE OVERRIDE (Always Dark) */
        [data-ui="terminal"] {{
            --bg: #000 !important; --text: #00ff41 !important; --sub: #008f11 !important; 
            --border: #00ff41 !important; --accent: #00ff41 !important;
        }}
        ...
        .sandwich {{ cursor: pointer; z-index: 2100; display: flex; flex-direction: column; gap: 4px; padding: 5px; }}
        .sandwich span {{ width: 18px; height: 2px; background: var(--text); transition: 0.3s; }}

        .nav-actions {{ display: flex; align-items: center; gap: 15px; }}
        .theme-btn {{ background: none; border: none; cursor: pointer; font-size: 16px; padding: 5px; color: var(--text); }}
        select {{ background: var(--border); color: var(--text); border: none; padding: 5px 8px; border-radius: 6px; font-size: 10px; font-weight: 800; cursor: pointer; outline: none; }}

        @media (max-width: 768px) {{
            .nav-actions {{
                position: fixed; top: 0; right: -100%; width: 220px; height: 100vh;
                background: var(--menu-bg); border-left: 1px solid var(--border);
                flex-direction: column; padding: 100px 20px; gap: 30px; transition: 0.4s cubic-bezier(0.4, 0, 0.2, 1);
            }}
        ...
        [data-ui="liquid"] .news-item {{ background: var(--bg); border: 1px solid var(--border); border-radius: 16px; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); }}
        [data-ui="terminal"] .items {{ font-family: 'JetBrains Mono', monospace; }}
        [data-ui="terminal"] .news-item {{ border: 1px solid var(--border); padding: 12px; }}
        ...
        .title a {{ text-decoration: none; color: inherit; transition: 0.2s; }}
...
<body>
    ...
    <script>
        ...
        function toggleTheme() {{
            ...
            localStorage.setItem('nius-v15-theme', target);
        }}

        function setUI(mode) {{
            document.documentElement.setAttribute('data-ui', mode);
            localStorage.setItem('nius-v15-ui', mode);
        ...
        window.onload = () => {{
            updateDate();
            setUI(localStorage.getItem('nius-v15-ui') || 'liquid');
            const theme = localStorage.getItem('nius-v15-theme') || 
