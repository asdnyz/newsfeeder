...
TECH_KEYWORDS = ["AI", "Nvidia", "Apple", "GPT", "OpenAI", "Microsoft", "LLM", "Silicon", "Tesla", "Fintech", "Market", "Economy"]
...
def fetch_news():
    ...
    content = ""
    for entry in all_entries[:6]: # Shows 6 stories in a 3-column grid
...
def generate_index_html(content):
    ...
    raw_stories = [s.strip() for s in content.split('---') if s.strip()]
    cards_html = ""
    for story in raw_stories:
        ...
        url = re.search(r'\((https?://\S+)\)', title_line).group(1) if '(' in title_line else "#"
        title_text = re.sub(r'\[|\]|\(https?://\S+\)', '', title_line).strip()
        
        details = lines[1].lstrip('- ') if len(lines) > 1 else ""
        ...
        <div class="VPFeature">
            <article class="box">
                ...
                <a href="{url}" target="_blank" class="VPLink link">{link_icon} Open Source</a>
            </article>
        </div>"""
...
<head>
    ...
    <style>
        ...
        /* Navigation */
        .VPNav {{
            position: fixed; top: 0; left: 0; width: 100%; height: var(--vp-nav-height);
            background-color: var(--vp-c-bg); border-bottom: 1px solid var(--vp-c-border);
        ...
        /* Grid */
        .container {{ max-width: 1152px; margin: 100px auto; padding: 0 24px; }}
        .items {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 24px; }}
        
        .box {{
            display: flex; flex-direction: column; height: 100%;
            padding: 24px; border-radius: 12px; border: 1px solid var(--vp-c-bg-soft);
            background-color: var(--vp-c-bg-soft); transition: border-color 0.25s, background-color 0.25s;
        }}
        .box:hover {{ border-color: var(--vp-c-brand); }}
        .title {{ line-height: 24px; font-size: 16px; font-weight: 600; margin: 0 0 8px; }}
        .details {{ flex-grow: 1; line-height: 24px; font-size: 14px; font-weight: 500; color: var(--vp-c-text-2); margin-bottom: 20px; }}
        .VPLink {{ font-size: 12px; font-weight: 600; color: var(--vp-c-brand); text-decoration: none; display: flex; align-items: center; gap: 4px; }}
        .link-icon {{ width: 14px; height: 14px; }}
        b {{ color: var(--vp-c-text-1); font-weight: 700; }}

        @media (max-width: 768px) {{
            .nav-center {{ display: none; }}
            .VPNavBar {{ padding: 0 24px; }}
            .container {{ padding: 0 16px; margin-top: 80px; }}
        }}
    </style>
</head>
<body>
